        ...
        <div >
            <div className="shadow rounded-lg flex">
                <div className="relative">
                    <button
                        onClick={() => setToggle(!toggle)} className="rounded-lg inline-flex items-center bg-white hover:text-blue-500 focus:outline-none focus:shadow-outline text-gray-500 font-semibold py-2 px-2 md:px-4">
                        ...
                        <span className="hidden md:block">{state.filter || "Tất cả"}</span>
...
const Option = ({ value, setToggle, toggle }) => {
    ...
    return (
        <Link to={`?sort=${(value)}`}>
            <div onClick={() => handleClick()} className="flex justify-end px-4 hover:bg-gray-200 active:bg-gray-300">{value}</div>
        </Link>
    )
}
