...
const NextIcon = ({ currentPage, totalPage }) => {
    ...
    return (
        (currentPage !== totalPage)
            ? (<Link to={`?page=${currentPage + 1}`}>
                <span data-v-6b3fd699 data-v-b2c24e40 onClick={() => nextPage()} className={className} style={{ minHeight: '40px', minWidth: '40px' }}>
                    <div data-v-6b3fd699="" className="bg opacity-0 hover:opacity-20 active:opacity-40 dark:hover:opacity-30 dark:active:opacity-10 bg-shade-mid"></div>
...
const CurrentPageIcon = ({ page }) => {
    return (
        <span data-v-6b3fd699 data-v-b2c24e40 className="rounded relative md-btn flex items-center px-3 justify-center text-black bg-primary hover:bg-orange-600 active:bg-orange-700 dark:bg-accent-lighten2 dark:hover:bg-accent-lighten dark:active:bg-accent" style={{ minHeight: '40px', minWidth: '40px' }}>
            <span data-v-6b3fd699="" className="flex items-center justify-center font-medium select-none" style={{ pointerEvents: 'none' }}>{page}</span>
        </span>
    )
}
