        ...
        <div tabIndex={0} className="md-select focus:outline-none" onPointerLeave={() => {if(toggle) setToggle(!toggle)}} onClick={() => setToggle(!toggle)} >
            <div className={className}> {/*class rounded change to rounded-t when clicked*/}
                ...
                <div className="flex-grow relative py-[0.3125rem]">
                    ...
                    <div className="absolute top-4 transition-label with-placeholder" >{'sad'}</div> {/*Add class populated to show*/}
                    <div className="placeholder-text opacity-40 with-label" style={{ minHeight: '1rem' }}>{state.user.role || 'user'}</div> {/*Add class dropdown when clicked*/}
                </div>
            ...
            <div className="relative overflow-y-auto overscroll-contain z-10 bg-accent shadow rounded-b" style={{ minWidth: '100%', maxWidth: '100%', display: display }}> {/*Display : 'none' to close option */}
                {
                    ...
                    ?(roles.map((role, index) => (
                        (role === state.user.role)
                            ? (<SelectedOption key={index} value={role} ></SelectedOption>)
                            : (<Option key={index} value={role} ></Option>)
                    )))
                    :(roles.map((role, index) => (<Option key={index} value={role} ></Option>)))
                }
            </div>
            {/*<!---->*/}
        </div>
...
const Option = ({ value }) => {
    ...
    const handleClick = ()=>{
        setState({...state,user:{...state.user,role:value}})
    }
